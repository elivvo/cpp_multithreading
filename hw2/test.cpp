...
double ElapsedTime(std::chrono::high_resolution_clock::time_point start) {
  ...
  duration<double> time_span =
      std::chrono::duration_cast<duration<double>>(Now() - start);
...
TEST(Correctness, Simple) { RunTest(1, 1, 200); }

TEST(Correctness, Senders) { RunTest(4, 1, 200); }

TEST(Correctness, Receivers) { RunTest(1, 6, 200); }

TEST(Correctness, BigBuf) { RunTest(3, 3, 200); }
...
TEST(Block, Sender) { BlockRun(BlockType::kSender); }

TEST(Block, Receiver) { BlockRun(BlockType::kReceiver); }